...
class SelM_R50(nn.Module):
    ...
    def forward(self, x_in, audio_embed):

        x = self.resnet.conv1(x_in)
        x = self.resnet.bn1(x)
        ...
        audio_feature = audio_feature.squeeze(2).view(-1, 5, 256).contiguous()

        feature_map_list, audio_feature = self.BCSM(
            [x1, x2, x3, x4], audio_feature)

        fuse_mask, maps = self.decoder(
            feature_map_list, audio_feature, audio_mask)

        fuse_mask = F.interpolate(
            fuse_mask, (224, 224), mode='bilinear', align_corners=True)
...
class SelM_PVT(nn.Module):
    ...
    def forward(self, x_in, audio_embed):
        ...
        x1, x2, x3, x4 = self.encoder_backbone(x_in, audio_embed, audio_mask)
        ...
        audio_embed = audio_embed.view(-1, 5, 256)
        feature_map_list, audio_embed = self.tem_att(
            [x1, x2, x3, x4], audio_embed)


        fuse_mask, maps = self.decoder(
            feature_map_list, audio_embed, audio_mask)
        fuse_mask = F.interpolate(
            fuse_mask, (224, 224), mode='bilinear', align_corners=True)
