...
from avs_ms3.model.DAM import DAM_Fusion_Block,No_Fusion_Block,ALL_Fusion_Block_Add
...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def forward_features(self, x,audio,audio_mask):
        B = x.shape[0]
        outs = []
        ...
        for i in range(self.num_stages):
            ...
            x,audio = fusion(x,audio,audio_mask)
            outs.append([x,audio])

        return outs

    def forward(self, x,audio,audio_mask):
        x = self.forward_features(x,audio,audio_mask)
        # x = self.head(x)
...
def pvt_v2_b5(pretrained=False, **kwargs):
    ...
    for p in model.DAM_Fusion1.parameters():
        p.requires_grad=True
    for p in model.DAM_Fusion2.parameters():
        p.requires_grad=True
    for p in model.DAM_Fusion3.parameters():
        p.requires_grad=True
    for p in model.DAM_Fusion4.parameters():
        p.requires_grad=True    
    return model
